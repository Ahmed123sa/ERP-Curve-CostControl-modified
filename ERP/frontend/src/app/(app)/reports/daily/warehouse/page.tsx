...
export default function WarehouseDailyPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['warehouse-daily', month, selectedWh],
    queryFn: () => api.get('/reports/warehouse-daily', {
  ...
  const report = data as {
    month: string; days_in_month: number;
  ...
  return (
    <div className="flex flex-col h-full bg-gray-50/50" dir="rtl">
      <PageHeader title="اليوميات — وارد المخازن" />
      ...
      {!selectedWh && (
        <div className="p-8 text-center text-gray-400">اختر المخزن والشهر لعرض التقرير</div>
      )}
