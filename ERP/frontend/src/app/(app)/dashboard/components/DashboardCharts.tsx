...
export function DiffPieChart({ data, isLoading }: DiffPieProps) {
  ...
  return (
    <Card className="border-gray-100 dark:border-gray-700/50 shadow-sm">
      <CardHeader className="pb-2 px-4 pt-4">
        <CardTitle className="text-xs font-medium text-gray-500 dark:text-gray-400">توزيع الفروق حسب المخازن</CardTitle>
      </CardHeader>
...
export function TopDiffItems({ data, isLoading }: TopItemsProps) {
  ...
  return (
    <Card className="border-gray-100 dark:border-gray-700/50 shadow-sm">
      <CardHeader className="pb-2 px-4 pt-4">
        <CardTitle className="text-xs font-medium text-gray-500 dark:text-gray-400">أعلى 10 أصناف في الفروق</CardTitle>
      </CardHeader>
      <CardContent className="px-2 pb-2">
          ...
          {data.map((d) => {
            ...
            return (
              <div key={d.item_name} className="flex items-center gap-2">
                <span className="text-[11px] text-gray-600 dark:text-gray-300 w-24 truncate text-right">{d.item_name}</span>
                ...
                <span className={`text-[11px] font-mono w-16 text-left ${isNeg ? 'text-red-600 dark:text-red-400' : 'text-green-600 dark:text-green-400'}`}>
...
export function TrendChart({ data, isLoading }: TrendChartProps) {
  ...
  return (
    <Card className="border-gray-100 dark:border-gray-700/50 shadow-sm col-span-full">
      <CardHeader className="pb-2 px-4 pt-4">
        <CardTitle className="text-xs font-medium text-gray-500 dark:text-gray-400">اتجاه شهري (آخر 6 أشهر)</CardTitle>
      </CardHeader>
      <CardContent className="px-2 pb-2">
...
export function CardSkeleton() {
  return (
    <Card className="border-gray-100 dark:border-gray-700/50 shadow-sm animate-pulse">
      <CardContent className="p-4">
        <div className="h-4 bg-gray-100 dark:bg-gray-700 rounded w-1/2 mb-2" />
        <div className="h-32 bg-gray-50 dark:bg-gray-800 rounded" />
      </CardContent>
    </Card>
  );
}
  ...
  return (
    <Card className="border-gray-100 dark:border-gray-700/50 shadow-sm">
      ...
      <CardContent className="px-4 pb-4">
        <div className="text-center text-gray-300 dark:text-gray-600 text-sm py-6">لا توجد بيانات كافية</div>
      </CardContent>
    </Card>
  );
}