...
export default function ClientDiffsPage() {
  const month = new Date().toISOString().slice(0, 7);

  const { data: summary, isLoading } = useQuery({
    queryKey: ['client-diffs', month],
    queryFn: () => api.get('/client/stock/warehouse-summary', { params: { month } }).then((r) => r.data),
  });

  return (
    <div className="flex-1 overflow-y-auto p-6">
      <h1 className="text-xl font-bold text-gray-900 dark:text-gray-100 mb-4">الفروق والهدر</h1>
            ...
            {isLoading ? (
              ...
              summary?.map((w: any, i: number) => (
                <tr key={i} className="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                  ...
                  <td className="px-4 py-3 font-mono">{w.opening.toFixed(2)}</td>
                  <td className="px-4 py-3 font-mono">{typeof w.purchases === 'string' ? w.purchases : w.purchases.toFixed(2)}</td>
                  <td className={`px-4 py-3 font-mono ${w.diff < 0 ? 'text-red-600' : w.diff > 0 ? 'text-amber-600' : ''}`}>
                    {w.diff.toFixed(2)}
                  </td>
                </tr>
              ))
