...
export default function ClientFinancialPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['client-report-financial', month],
    queryFn: () => api.get('/client/reports/financial', { params: { month } }).then((r) => r.data),
  });

  return (
    <div className="flex-1 overflow-y-auto p-6">
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-xl font-bold text-gray-900 dark:text-gray-100">التقرير المالي</h1>
        <div className="flex items-center gap-2">
          ...
          <ExportButtons baseUrl="/api/client/reports/financial" params={{ month }} disabled={!data?.warehouses?.length} />
        </div>
      </div>
      ...
      {data?.financial && (
        <div className="grid grid-cols-3 gap-4 mb-4">
          <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 p-4 text-center">
            ...
            <div className="text-lg font-bold text-emerald-600">{data.financial.total_sales.toFixed(2)}</div>
          </div>
          <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 p-4 text-center">
            <div className="text-xs text-gray-500 mb-1">إجمالي المصروفات</div>
            <div className="text-lg font-bold text-red-600">{data.financial.total_expenses.toFixed(2)}</div>
          </div>
          <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 p-4 text-center">
            <div className="text-xs text-gray-500 mb-1">صافي الربح</div>
            <div className={`text-lg font-bold ${data.financial.net_total >= 0 ? 'text-emerald-600' : 'text-red-600'}`}>{data.financial.net_total.toFixed(2)}</div>
          </div>
        </div>
      )}
