...
export default function ClientAnalyticsPage() {
  ...
  const { data: warehouses } = useQuery({
    queryKey: ['client-wh-list'],
    queryFn: () => api.get('/client/warehouses').then((r) => r.data),
  });
  const inventoryWh = (warehouses || []).filter((w: any) => w.type !== 'branch');

  const { data: alerts } = useQuery({
    queryKey: ['client-analytics-inventory', selectedWarehouses],
    queryFn: () => api.get('/client/analytics/inventory-alerts', {
  ...
  const { data: purchases } = useQuery({
    queryKey: ['client-analytics-purchases', limit, purchasesWhId, purchasesMonth],
    queryFn: () => api.get('/client/analytics/top-purchases', {
  ...
  const { data: prices } = useQuery({
    queryKey: ['client-analytics-prices', threshold, pricesMonth],
    queryFn: () => api.get('/client/analytics/price-changes', {
  ...
  const { data: costImpact } = useQuery({
    queryKey: ['client-analytics-cost-impact', costImpactMonth],
    queryFn: () => api.get('/client/analytics/cost-impact', {
  ...
  const { data: stockValue } = useQuery({
    queryKey: ['client-analytics-stock-value'],
    queryFn: () => api.get('/client/analytics/stock-value').then((r) => r.data),
  });

  return (
    <div className="flex-1 flex flex-col h-full bg-gray-50/50 dark:bg-gray-950" dir="rtl">
      <div className="bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 px-6 py-4">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-lg font-bold text-gray-900 dark:text-gray-100">🔬 التحليلات الذكية</h2>
          ...
          {activeTab === 'prices' && (
            <div className="flex items-center gap-2">
              ...
              <input type="number" value={threshold} onChange={(e) => setThreshold(e.target.value)}
                className="w-16 border border-gray-200 dark:border-gray-700 rounded px-2 py-1 text-xs text-center outline-none dark:bg-gray-800 dark:text-gray-200" />
          ...
          {activeTab === 'purchases' && (
            <div className="flex items-center gap-2">
              ...
              <select value={limit} onChange={(e) => setLimit(e.target.value)}
                className="border border-gray-200 dark:border-gray-700 rounded px-2 py-1 text-xs outline-none dark:bg-gray-800 dark:text-gray-200">
      ...
      <div className="bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 px-6 py-0 flex gap-0 overflow-x-auto">
        {tabs.map((t) => (
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-4">
              ...
              {inventoryWh.map((wh: any) => {
                ...
                return (
                  <button key={wh.id} onClick={() => setSelectedWarehouses(prev =>
                    ...
                    active ? 'bg-blue-600 text-white border-blue-600' : 'bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-400 border-gray-300 dark:border-gray-700 hover:border-blue-400'
                  }`}>
                    {wh.name}
                  </button>
                );
        ...
        {activeTab === 'purchases' && (
          <>
            <div className="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-xl p-3 shadow-sm flex items-center gap-3 flex-wrap">
              ...
              <select value={purchasesWhId} onChange={(e) => setPurchasesWhId(e.target.value)}
                className="border border-gray-200 dark:border-gray-700 rounded-lg px-3 py-2 text-xs outline-none bg-white dark:bg-gray-800 dark:text-gray-200">
              ...
              <input type="month" value={purchasesMonth} onChange={(e) => setPurchasesMonth(e.target.value)}
                className="border border-gray-200 dark:border-gray-700 rounded-lg px-3 py-2 text-xs outline-none dark:bg-gray-800 dark:text-gray-200" />
        ...
        {activeTab === 'prices' && (
          <>
            <div className="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-xl p-3 shadow-sm flex items-center gap-3 flex-wrap">
              ...
              <input type="number" value={threshold} onChange={(e) => setThreshold(e.target.value)}
                className="border border-gray-200 dark:border-gray-700 rounded-lg px-3 py-2 text-xs outline-none w-20 dark:bg-gray-800 dark:text-gray-200" />%
              {pricesMonth && (
...
function InventoryAlertsTab({ data }: { data: any }) {
  ...
  const Bar = ({ pct }: { pct: number }) => {
    ...
    return (
      <div className="bg-gray-100 dark:bg-gray-800 rounded-full h-2.5 w-24 overflow-hidden inline-block align-middle">
  ...
  const Tables = ({ items, color, label }: { items: any[]; color: string; label: string }) => (
    <div className="bg-white dark:bg-gray-900 border rounded-xl shadow-sm overflow-hidden">
      ...
      <table className="w-full text-sm">
        <thead><tr className="text-xs text-gray-500 border-b border-gray-50 dark:border-gray-800">
          ...
          {items.map((a: any, i: number) => {
            ...
            return (
              <tr key={i} className="hover:bg-gray-50/30 dark:hover:bg-gray-800/30">
                <td className="px-3 py-2 font-medium text-gray-800 dark:text-gray-200">{a.item_name}</td>
                ...
                <td className="px-3 py-2 text-end">
                  {a.days_until_stockout !== null ? (
                    <span className={`font-mono font-bold text-xs ${urgent ? 'text-red-600 bg-red-50 dark:bg-red-900/30 px-1.5 py-0.5 rounded' : 'text-gray-500'}`}>
  ...
  return (
    <>
      ...
      {!data.out_of_stock?.length && !data.critical?.length && !data.warning?.length && (
        <div className="bg-white dark:bg-gray-900 border border-green-200 dark:border-green-800 rounded-xl p-8 text-center text-green-600 font-medium">
  ...
  return (
    <>
      <div className="bg-gradient-to-r from-blue-600 to-blue-500 rounded-xl p-5 shadow text-white">
        ...
        <div className="text-3xl font-bold mt-1">{data.total_value_all?.toLocaleString() ?? 0} ج</div>
      </div>

      <div className="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-xl shadow-sm overflow-hidden">
        <div className="px-4 py-3 border-b border-gray-100 dark:border-gray-800 font-medium text-gray-700 dark:text-gray-300 text-sm flex items-center gap-2">
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-gray-50 dark:divide-gray-800">
            {data.items?.map((p: any) => (
              <tr key={p.rank} className="hover:bg-gray-50/30 dark:hover:bg-gray-800/30">
                <td className="px-3 py-2 text-center text-gray-400 text-xs">{p.rank}</td>
                <td className="px-3 py-2 font-medium text-gray-800 dark:text-gray-200">{p.item_name}</td>
                <td className="px-3 py-2 text-gray-400 text-xs">{p.category || '—'}</td>
                <td className="px-3 py-2 text-end font-mono text-gray-700 dark:text-gray-300">{p.purchase_count}</td>
                <td className="px-3 py-2 text-end font-mono text-gray-700 dark:text-gray-300">{p.total_qty} {p.unit}</td>
                <td className="px-3 py-2 text-end font-mono text-green-700 dark:text-green-400 font-medium">{p.total_value?.toLocaleString()} ج</td>
                <td className="px-3 py-2 text-end">
                  <div className="flex items-center gap-2 justify-end">
                    <span className="font-mono text-xs text-gray-500">{p.contribution_pct}%</span>
                    <div className="bg-gray-100 dark:bg-gray-800 rounded-full h-2 w-12 overflow-hidden">
    ...
    <>
      <div className={`rounded-xl p-5 shadow text-white ${data.net_impact >= 0 ? 'bg-gradient-to-r from-red-600 to-orange-500' : 'bg-gradient-to-r from-green-600 to-teal-500'}`}>
        ...
        <div className="text-3xl font-bold mt-1">
          {data.net_impact >= 0 ? '+' : ''}{data.net_impact?.toLocaleString() ?? 0} ج
        </div>
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-gray-50 dark:divide-gray-800">
            {data.changes?.map((c: any, i: number) => (
              <tr key={i} className="hover:bg-gray-50/30 dark:hover:bg-gray-800/30">
                <td className="px-3 py-2 font-medium text-gray-800 dark:text-gray-200">{c.item_name}</td>
                ...
                <td className="px-3 py-2 text-end font-mono font-bold text-blue-700 dark:text-blue-400">{c.avg_cost?.toFixed(2) ?? '—'}</td>
...
function CostImpactTab({ data }: { data: any }) {
  ...
  if (!data.menus?.length) return (
    <div className="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-xl p-8 text-center text-gray-400">
      لا توجد منيوات نشطة
    </div>
  );
    ...
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {data.menus.map((menu: any) => {
          ...
          return (
            <div key={menu.menu_id} className="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-xl shadow-sm overflow-hidden">
              <button onClick={() => setExpandedMenu(expanded ? null : menu.menu_id)}
                className="w-full text-right p-4 hover:bg-gray-50/50 dark:hover:bg-gray-800/50 transition-colors">
                <div className="flex items-start justify-between">
                  <div>
                    <div className="font-bold text-gray-800 dark:text-gray-200">🍽️ {menu.menu_name}</div>
                ...
                <div className="mt-3 flex items-center gap-2 text-xs text-gray-500">
                  <span className="bg-gray-100 dark:bg-gray-800 px-2 py-0.5 rounded">FC: {menu.old_fc_pct}%</span>
                  <span className="text-gray-300">→</span>
                  <span className={`px-2 py-0.5 rounded font-medium ${menuUp ? 'bg-red-50 dark:bg-red-900/30 text-red-600' : 'bg-green-50 dark:bg-green-900/30 text-green-600'}`}>
              ...
              {expanded && (
                <div className="border-t border-gray-100 dark:border-gray-800 px-4 py-3 space-y-3">
                  {(!menu.affected_recipes || menu.affected_recipes.length === 0) ? (
                    ...
                    menu.affected_recipes.map((r: any, j: number) => (
                      <div key={j} className="border border-gray-100 dark:border-gray-800 rounded-lg overflow-hidden">
                        <div className="flex items-center justify-between px-3 py-2 bg-gray-50/70 dark:bg-gray-800/50">
                          <div className="flex items-center gap-2">
                            <span className="font-medium text-gray-800 dark:text-gray-200 text-sm">🍳 {r.recipe_name}</span>
                        ...
                        {r.ingredients?.length > 0 && (
                          <table className="w-full text-xs">
                            <thead><tr className="text-gray-400 border-b border-gray-50 dark:border-gray-800">
                            ...
                            <tbody className="divide-y divide-gray-50 dark:divide-gray-800">
                              {r.ingredients.map((ing: any, k: number) => (
                                <tr key={k} className="hover:bg-gray-50/30 dark:hover:bg-gray-800/30">
                                  <td className="px-3 py-1 text-gray-600 dark:text-gray-400 pr-8">{ing.ingredient_name}</td>
    ...
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {data.warehouses?.map((wh: any) => (
          <div key={wh.warehouse_id} className="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-xl p-5 shadow-sm">
            <div className="flex items-center justify-between">
              <div>
                <div className="font-bold text-gray-800 dark:text-gray-200">{wh.name}</div>
...
function ParetoTab() {
  ...
  return (
    <>
      <div className="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-xl p-4 shadow-sm flex gap-3">
        <select value={branchId} onChange={(e) => { setBranchId(e.target.value); setMenuId(''); }}
          className="border border-gray-200 dark:border-gray-700 rounded-lg px-3 py-1.5 text-sm outline-none flex-1 dark:bg-gray-800 dark:text-gray-200">
      ...
      {pareto?.recipes?.map((recipe: any, i: number) => (
        <div key={recipe.recipe_id || i} className="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-xl shadow-sm overflow-hidden">
          <div className="bg-gray-50 dark:bg-gray-800/50 px-4 py-3 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between">
            ...
            {recipe.food_cost_pct > 35 && (
              <span className="text-xs bg-red-100 dark:bg-red-900/30 text-red-600 px-2 py-0.5 rounded-full">⚠️ مرتفع</span>
            )}
          </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-50 dark:divide-gray-800">
              {recipe.ingredients?.map((ing: any, j: number) => (
                <tr key={j} className={`hover:bg-gray-50/30 dark:hover:bg-gray-800/30 ${ing.pareto_group === 'A' ? 'bg-red-50/30 dark:bg-red-900/10' : ing.pareto_group === 'B' ? 'bg-amber-50/30 dark:bg-amber-900/10' : ''}`}>
                  <td className="px-3 py-2 text-center">
                    <span className={`text-xs font-bold px-1.5 py-0.5 rounded ${
                      ing.pareto_group === 'A' ? 'bg-red-100 dark:bg-red-900/40 text-red-700 dark:text-red-400' :
                      ing.pareto_group === 'B' ? 'bg-amber-100 dark:bg-amber-900/40 text-amber-700 dark:text-amber-400' : 'bg-gray-100 dark:bg-gray-800 text-gray-500'
                    }`}>{ing.pareto_group}</span>
                  </td>
                  <td className="px-3 py-2 font-medium text-gray-800 dark:text-gray-200">{ing.ingredient_name}</td>
                  <td className="px-3 py-2 text-end font-mono text-gray-700 dark:text-gray-300">{ing.line_total?.toFixed(2)} ج</td>
                  <td className="px-3 py-2 text-end font-mono font-medium text-gray-700 dark:text-gray-300">{ing.pct}%</td>
