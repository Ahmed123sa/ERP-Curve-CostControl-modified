...
return new class extends Migration
{
    public function up(): void
    {
        ...
        Schema::create('users', function (Blueprint $table) {
            ...
            $table->enum('role', ['admin', 'cost_controller', 'viewer'])->default('cost_controller');
            $table->timestamps();
        });
        ...
        Schema::create('item_mappings', function (Blueprint $table) {
            $table->uuid('id')->primary()->default(DB::raw('gen_random_uuid()'));
            $table->uuid('client_id');
            $table->string('source_name');                  // "موتزريلا نص دسم" (في الإذن)
            $table->uuid('item_id');                        // الصنف الفعلي في الـ DB
            $table->string('context')->nullable();          // اسم المخزن أو الفرع
        ...
        Schema::create('location_mappings', function (Blueprint $table) {
            $table->uuid('id')->primary()->default(DB::raw('gen_random_uuid()'));
            $table->uuid('client_id');
            $table->string('source_name');                  // "وارد مخزن" (في الإذن)
        ...
        DB::statement('ALTER TABLE branches ENABLE ROW LEVEL SECURITY');
        DB::statement('ALTER TABLE dispatch_orders ENABLE ROW LEVEL SECURITY');
        DB::statement('ALTER TABLE dispatch_lines ENABLE ROW LEVEL SECURITY');
        DB::statement('ALTER TABLE stock_ledger ENABLE ROW LEVEL SECURITY');
        DB::statement('ALTER TABLE monthly_closings ENABLE ROW LEVEL SECURITY');
    }
