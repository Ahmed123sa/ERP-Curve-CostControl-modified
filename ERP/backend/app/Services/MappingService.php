...
class MappingService
{
    ...
    public function findLocation(string $clientId, string $sourceName): array
    {
        ...
        foreach ($warehouses as $wh) {
            if ($this->normalize($wh->name) === $normalized) {
        ...
        foreach ($branches as $br) {
            if ($this->normalize($br->name) === $normalized) {
        ...
        // 4. تطابق جزئي
        $allLocations = collect($warehouses->map(fn($w) => ['type'=>'warehouse','obj'=>$w]))
            ->concat($branches->map(fn($b) => ['type'=>'branch','obj'=>$b]));

        $bestMatch = null;
        $bestScore = 0;
        foreach ($allLocations as $loc) {
            $score = $this->similarityScore($normalized, $this->normalize($loc['obj']->name));
